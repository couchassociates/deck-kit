...
6. Tell the person: open the file in a normal browser tab to present; click the faint screen icon in the bottom right corner (or add `?presenter` to the address) for notes, clock and tools in a second window; share the plain window. The tools work on slides, not on the index. A chat preview pane shows the slides but cannot run the second window.
