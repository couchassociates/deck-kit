...
- **A visible way into the presenter view.** `?presenter` and N were only known to people who had read the docs. `#pk-present` is a fixed button in the bottom right corner of a plain window: 30% opacity on slides because the audience sees that window, 60% on the index, full on hover, hidden in the presenter view. It calls `window.open` on the same URL with `?presenter` and the current `#/n`, named `deck-presenter` so a second click reuses the window. When that returns nothing (popup blocked, sandboxed pane, a protocol other than http, https or file) it toggles the panel in place. The HUD moved left to make room. A deck hides it with plain CSS rather than a new attribute, so the contract did not grow.
