(function(){
  ...
  grid.innerHTML='<div class="gridhead"><h2>'+DECK_TITLE+' <span class="mute" style="font-weight:400;font-size:34px">· '+content.length+' slides'+(version?' · '+version:'')+'</span></h2><div class="hint">Click a slide · → to start · ? how to present</div></div><div class="thumbs" id="thumbs"></div><aside class="notes"><p>This is the index. The deck loops: after the last slide you land back here. Press → to begin on the title slide.</p><p>The drawing tools start working on the first slide. Pick one now and it is ready when you advance.</p><p>Presenting to others? Click "Open a window to share" below and share that window. Press ? for the full guide.</p></aside>';
  ...
  function fitTools(){
    ...
    tools.classList.remove('compact','tight');
    if(tools.scrollWidth>tools.clientWidth+1)tools.classList.add('compact');
    if(tools.scrollWidth>tools.clientWidth+1)tools.classList.add('tight');
  }
  ...
  function inStep(){document.getElementById('syncState').textContent='in step';document.getElementById('sync').classList.add('paired');}
  function applyPeer(msg){
    if(!msg||typeof msg.i!=='number')return;
    fromPeer=true;go(msg.i);setHold(msg.hold);fromPeer=false;
    /* a plain window that is being driven is the one on show: keep the corner buttons faint and unlabelled there */
    if(!presenterMode())body.classList.add('pk-following');
    inStep();
  }
  if(chan){chan.onmessage=function(e){
    ...
    if(d.bye){document.getElementById('syncState').textContent='solo';document.getElementById('sync').classList.remove('paired');return;}
  ...
  document.addEventListener('keydown',function(e){
    ...
    if(help.classList.contains('open')){if(k==='Escape'||k==='?'){e.preventDefault();toggleHelp(false);}return;}
  ...
  /* one button, both directions. In a plain window it sits in the corner; in the presenter view it moves to the end of
     the toolbar, where fitTools() can account for it, and reads "Exit presenter". */
  var presentBtn=document.getElementById('pk-present'),helpBtn=document.getElementById('pk-helpbtn'),help=document.getElementById('pk-help'),helpGo=document.getElementById('pk-helpgo');
  function toggleHelp(show){var on=show===undefined?!help.classList.contains('open'):show;help.classList.toggle('open',on);if(on)helpGo.textContent=presenterMode()?'Open a window to share':'Open presenter view';}
  function placePresent(){
    var on=body.classList.contains('notes-open');
    var home=on?tools:document.getElementById('pk-corner');home.appendChild(helpBtn);home.appendChild(presentBtn);
    if(on)body.classList.remove('pk-following');
    presentBtn.setAttribute('aria-pressed',on?'true':'false');
    presentBtn.setAttribute('aria-label',on?'Exit the presenter view':'Open the presenter view');
    presentBtn.title=on?'Back to the slides only (N)':'Presenter view: notes, clock and tools (N)';
    presentBtn.querySelector('.lbl').textContent=on?'Exit presenter':'Presenter view';
  }
  function toggleNotes(){body.classList.toggle('notes-open');placePresent();fit();renderNext();markRibbon();}
  /* the window to share: this deck again without the notes, on the same slide. It says hello on load and the two fall in step. */
  function openAudience(){
    var w=null;
    if(/^(https?|file):$/.test(location.protocol)){
      var u=location.href.split('#')[0].replace(/([?&])presenter(=[^&]*)?(&|$)/,function(m,a,v,z){return z?a:'';});
      try{w=window.open(u+'#/'+cur,'deck-audience','width=1280,height=760');}catch(e){}
    }
    if(!w)document.getElementById('pkAudience').textContent='Blocked here. Open this file in a second window.';
  }
  ...
  presentBtn.addEventListener('click',function(e){e.stopPropagation();this.blur();toggleNotes();});
  helpBtn.addEventListener('click',function(e){e.stopPropagation();this.blur();toggleHelp();});
  document.getElementById('pk-helpclose').addEventListener('click',function(){toggleHelp(false);});
  help.addEventListener('click',function(e){e.stopPropagation();if(e.target===help)toggleHelp(false);});
  helpGo.addEventListener('click',function(){toggleHelp(false);if(presenterMode())openAudience();else toggleNotes();});
  document.getElementById('pkAudience').addEventListener('click',function(e){e.stopPropagation();this.blur();openAudience();});
  ...
  window.addEventListener('pagehide',function(){send({bye:1});});
  ...
  if(/[?&]presenter/.test(location.search)){body.classList.add('notes-open');placePresent();}
  ckLoad();
