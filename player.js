(function(){
  ...
  grid.innerHTML='<div class="gridhead"><h2>'+DECK_TITLE+' <span class="mute" style="font-weight:400;font-size:34px">· '+content.length+' slides'+(version?' · '+version:'')+'</span></h2><div class="hint">Click a slide · → to start · N notes · F fullscreen</div></div><div class="thumbs" id="thumbs"></div><aside class="notes"><p>This is the index. The deck loops: after the last slide you land back here. Press → to begin on the title slide.</p></aside>';
  ...
  function buildThumbs(){
    ...
    for(var i=1;i<N;i++){(function(i){
      ...
      var f=document.createElement('div');f.className='frame';f.appendChild(cloneSlide(i));
  ...
  function buildRibbon(){
    ...
    slides.forEach(function(sl,i){
      ...
      b.setAttribute('aria-label','Go to slide '+i+': '+titleOf(i));
      var f=document.createElement('div');f.className='rf';
  ...
  function go(i,push){
    ...
    if(push!==false){var h='#/'+cur;if(location.hash!==h)history.replaceState(null,'',h);}
    if(cur===0)fitThumbs();
    document.title=(cur===0?'Index':String(cur).padStart(2,'0')+' '+titleOf(cur))+' · '+DECK_TITLE;
    renderNext();markRibbon();renderClock();
  ...
  function applyPeer(msg){
    ...
    document.getElementById('syncState').textContent='in step';
  }
  if(chan){chan.onmessage=function(e){
    var d=e.data;if(!d)return;
    if(d.hello){if(body.classList.contains('notes-open')){send({inkAll:inkStore});broadcast();ckSave();document.getElementById('syncState').textContent='in step';}return;}
  ...
  function setTool(t){
    if(t===tool)t='none';
    tool=t;
    stage.classList.toggle('tool-on',t!=='pointer'&&t!=='none');stage.classList.toggle('tool-laser',t==='laser');
  ...
  window.__deck=function(){return {cur:cur,tool:tool,strokes:(inkStore[cur]||{strokes:[]}).strokes.length,ellipses:(inkStore[cur]||{strokes:[]}).strokes.filter(function(x){return x.tool==='ellipse';}).length,mag:!!(inkStore[cur]&&inkStore[cur].mag),presenter:body.classList.contains('notes-open'),remoteHover:remoteHover&&remoteHover.tool,remoteSel:remoteSel?remoteSel.length:0};};
  ...
  document.addEventListener('keydown',function(e){
    ...
    else if(k==='n'||k==='N'){e.preventDefault();body.classList.toggle('notes-open');fit();renderNext();markRibbon();}
  ...
  document.getElementById('btnNotes').addEventListener('click',function(){body.classList.toggle('notes-open');fit();renderNext();markRibbon();});
  ...
  if(!body.classList.contains('notes-open'))send({hello:1});
})();
